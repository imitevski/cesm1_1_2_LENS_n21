!===============================================================================
! SVN $Id: seq_timemgr_mod.F90 41525 2012-11-03 00:19:43Z tcraig $
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/drv/seq_mct/branch_tags/cesm1_1_0_rel_tags/cesm1_1_0_n03_drvseq4_2_08/shr/seq_timemgr_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
!EOP
   ...
   integer(SHR_KIND_IN),private,parameter :: max_clocks = 7
   character(len=*),public,parameter :: &
   ...
   integer(SHR_KIND_IN),private,parameter :: &
      ...
      seq_timemgr_nclock_rof  = 7
   character(len=8),private,parameter :: seq_timemgr_clocks(max_clocks) = &
      (/'drv     ','atm     ','lnd     ','ocn     ', &
        'ice     ','glc     ','rof     '/)

   integer(SHR_KIND_IN),private,parameter :: max_alarms = 14
   character(len=*),public,parameter :: &
   ...
   type EClock_pointer     ! needed for array of pointers
      type(ESMF_Clock),pointer :: EClock
...
subroutine seq_timemgr_clockInit(SyncClock, nmlfile, restart, restart_file, mpicom, &
      EClock_drv, EClock_atm, EClock_lnd, EClock_ocn, EClock_ice, Eclock_glc, Eclock_rof)
...
!-------------------------------------------------------------------------------
    ...
    if (iam == 0) then
       ...
       if ( atm_cpl_dt <= 0 .or. &
            ...
            ocn_cpl_dt <= 0 .or. glc_cpl_dt <= 0 .or. rof_cpl_dt <=0) then
          write(logunit,*) trim(subname),' ERROR: aliog _cpl_dt = ', &
       ...
       if ( abs(atm_cpl_offset) > atm_cpl_dt .or. &
          ...
          write(logunit,*) trim(subname),' ERROR: aliog _cpl_offset = ', &
             atm_cpl_offset, lnd_cpl_offset, ice_cpl_offset, ocn_cpl_offset, &
             glc_cpl_offset, rof_cpl_offset
          call shr_sys_abort( subname//': ERROR coupling intervals invalid' )
       end if
