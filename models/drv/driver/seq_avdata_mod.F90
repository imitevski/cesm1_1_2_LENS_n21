!===============================================================================
! SVN $Id: seq_avdata_mod.F90 40372 2012-09-18 23:34:49Z tcraig $
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/drv/seq_mct/branch_tags/cesm1_1_0_rel_tags/cesm1_1_0_n03_drvseq4_2_08/driver/seq_avdata_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
! !PUBLIC DATA MEMBERS:
   ...
   type(mct_gGrid) :: dom_aa(num_inst_atm)      ! atm domain on atm pes
   type(mct_gGrid) :: dom_ll(num_inst_lnd)      ! lnd domain
   type(mct_gGrid) :: dom_ii(num_inst_ice)      ! ice domain
   type(mct_gGrid) :: dom_oo(num_inst_ocn)      ! ocn domain
   type(mct_gGrid) :: dom_rr(num_inst_rof)      ! runoff domain
   type(mct_gGrid) :: dom_gg(num_inst_glc)      ! glc domain
   type(mct_gGrid) :: dom_ss(num_inst_lnd)      ! sno domain
   ...
   type(mct_aVect) :: l2r_rx                  ! Lnd export, rof grid, cpl pes - allocated in driver, temporary single instance
   ...
   type(mct_aVect) :: r2oacc_rx(num_inst_rof) ! Rof export, rof grid, cpl pes - allocated in driver
   type(mct_aVect) :: r2x_lx(num_inst_rof)    ! Rof export, lnd grid, lnd pes - allocated in lnd gc
   type(mct_aVect) :: r2o_ox(num_inst_rof)    ! Rof export, ocn grid, cpl pes - allocated in driver
   ...
   integer(IN)     :: r2oacc_rx_cnt ! r2oacc_rx: number of time samples accumulated
