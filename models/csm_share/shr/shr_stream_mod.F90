!===============================================================================
! SVN $Id: shr_stream_mod.F90 36506 2012-04-18 17:41:58Z tcraig $
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/csm_share/branch_tags/cesm1_1_1_LENS_control_tags/cesm1_1_1_LENS_control_n01_share3_121022/shr/shr_stream_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
!EOP
   ...
   type shr_stream_streamType
      ...
      integer  (SHR_KIND_IN),pointer :: initarr(:)! surrogate for init flag
...
!EOP
   ...
   character(SHR_KIND_CX) :: inpcx       ! input char
      ...
      do n=1,nFiles                     ! data specific to each file...
         ...
         if (haveData) then     ! ie. if arrays have been allocated
            ...
            if (trim(name) == trim(strm(k)%file(n)%name)) then
               write(s_logunit,F05) "reading time axis for stream restart filename ",k,n,' ',trim(name),' ',trim(strm(k)%file(n)%name)
               strm(k)%file(n)%nt = nt
               ...
               write(s_logunit,F05) "WARNING, skip time axis for stream restart filename ",k,n,' ',trim(name),' ',trim(strm(k)%file(n)%name)
               readok = .false.
