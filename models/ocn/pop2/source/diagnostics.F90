...
!
! !REVISION HISTORY:
!  SVN:$Id: diagnostics.F90 47397 2013-05-22 15:33:53Z jedwards $
!
! !USES:
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      tavg_HBLT_2,       &! tavg id for average boundary layer depth, stream #2 (allows two frequencies)
      tavg_XBLT,         &! tavg id for maximum boundary layer depth
      tavg_XBLT_2,       &! tavg id for maximum boundary layer depth, stream #2 (allows two frequencies)
...
!-----------------------------------------------------------------------
   ...
   call define_tavg_field(tavg_XBLT_2,'XBLT_2',2,                     &
                          tavg_method=tavg_method_max,                &
                          long_name='Maximum Boundary-Layer Depth',   &
                          units='centimeter', grid_loc='2110',        &
                          coordinates='TLONG TLAT time')
...
!-----------------------------------------------------------------------
        ...
        !$OMP PARALLEL DO PRIVATE(iblock)
        do iblock=1,nblocks_clinic
          call accumulate_tavg_field(KPP_HBLT(:,:,iblock), tavg_XBLT_2, iblock, 1)
        end do
        !$OMP END PARALLEL DO
