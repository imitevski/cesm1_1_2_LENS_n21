...
!-----------------------------------------------------------------------
   ...
   integer (int_kind), parameter :: &
      ...
      dic_alt_co2_ind  = 25     ! DIC with alternative CO2
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      tavg_CO2STAR_ALT_CO2,&! tavg id for co2star alternative CO2
      tavg_DCO2STAR,     &! tavg id for dco2star
      tavg_DCO2STAR_ALT_CO2,&! tavg id for dco2star alternative CO2
      tavg_pCO2SURF,     &! tavg id for surface pco2
      tavg_pCO2SURF_ALT_CO2,&! tavg id for surface pco2 alternative CO2
      tavg_DpCO2,        &! tavg id for delta pco2
      tavg_DpCO2_2,      &! tavg id for duplicate of delta pco2
      tavg_DpCO2_ALT_CO2,&! tavg id for delta pco2 alternative CO2
      ...
      tavg_DIC_GAS_FLUX_ALT_CO2, &! tavg id for dic flux alternative CO2
      tavg_O2_GAS_FLUX_2,&! tavg id for duplicate of O2 flux
      tavg_PH,           &! tavg id for surface pH
      tavg_PH_ALT_CO2,   &! tavg id for surface pH alternative CO2
      tavg_ATM_CO2,      &! tavg id for atmospheric CO2
      tavg_ATM_ALT_CO2,  &! tavg id for atmospheric alternative CO2
...
!-----------------------------------------------------------------------
   ...
   ind_name_table(25) = ind_name_pair(dic_alt_co2_ind, 'DIC_ALT_CO2')
...
!-----------------------------------------------------------------------
!  document starting surf_avg values
!-----------------------------------------------------------------------

   call document(subname, 'surf_avg(DIC)', surf_avg(dic_ind))
   call document(subname, 'surf_avg(ALK)', surf_avg(alk_ind))
   call document(subname, 'surf_avg(DIC_ALT_CO2)', surf_avg(dic_alt_co2_ind))
...
!-----------------------------------------------------------------------
   ...
   if (accumulate_tavg_now(tavg_CO3) .or. &
      ...
      if (lalt_co2_terms) then
         where (PH_PREV_ALT_CO2_3D(:,:,k,bid) /= c0)
            WORK1 = PH_PREV_ALT_CO2_3D(:,:,k,bid) - del_ph
            WORK2 = PH_PREV_ALT_CO2_3D(:,:,k,bid) + del_ph
         elsewhere
            WORK1 = phlo_3d_init
            WORK2 = phhi_3d_init
         end where
...
!-----------------------------------------------------------------------
            ...
            do j = 1,ny_block
               ...
               ECO_SFLUX_TAVG(:,j,17,iblock) = CO2STAR_ROW
               ECO_SFLUX_TAVG(:,j,18,iblock) = DCO2STAR_ROW
               ECO_SFLUX_TAVG(:,j,19,iblock) = pCO2SURF_ROW
               ECO_SFLUX_TAVG(:,j,20,iblock) = DpCO2_ROW
...
!-----------------------------------------------------------------------
            ...
            ECO_SFLUX_TAVG(:,:,21,iblock) = FLUX_ALT_CO2
            ECO_SFLUX_TAVG(:,:,22,iblock) = PH_PREV_ALT_CO2(:,:,iblock)
            ECO_SFLUX_TAVG(:,:,23,iblock) = XCO2_ALT_CO2
...
!-----------------------------------------------------------------------
      ...
      call accumulate_tavg_field(ECO_SFLUX_TAVG(:,:,17,iblock),  &
                                 tavg_CO2STAR_ALT_CO2,iblock,1)
      call accumulate_tavg_field(ECO_SFLUX_TAVG(:,:,18,iblock),  &
                                 tavg_DCO2STAR_ALT_CO2,iblock,1)
      call accumulate_tavg_field(ECO_SFLUX_TAVG(:,:,19,iblock),  &
                                 tavg_pCO2SURF_ALT_CO2,iblock,1)
      call accumulate_tavg_field(ECO_SFLUX_TAVG(:,:,20,iblock),  &
                                 tavg_DpCO2_ALT_CO2,iblock,1)
      call accumulate_tavg_field(ECO_SFLUX_TAVG(:,:,21,iblock),  &
                                 tavg_DIC_GAS_FLUX_ALT_CO2,iblock,1)
      call accumulate_tavg_field(ECO_SFLUX_TAVG(:,:,22,iblock),  &
                                 tavg_PH_ALT_CO2,iblock,1)
      call accumulate_tavg_field(ECO_SFLUX_TAVG(:,:,23,iblock),  &
